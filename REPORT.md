...
| ② 검수 실패 시 대안 | 그 건만 버리고 계속 진행 · 문체는 `style_tries`(5)까지 재요청 · 버릴 낱말 재검사 · 0건인 날도 발행 | 5-4절 층 표 |
...
### 내 도메인이 아닌 문화·공간으로 뉴스를 발간한 이유

이 뉴스레터는 일에 몰두하느라 일상의 여유를 갖지 못하는 사람을 위해 만듭니다.
주요 문화·전시·공연 정보는 이미 너무 많은 매체를 통해 쏟아지는데,
정작 눈에 띄는 것은 늘 화제작뿐입니다. 그래서 이 브리핑은 반대로 갑니다 —
화제작보다 덜 알려진 것 중에서 볼 만한 것과 가볼 만한 곳을 골라,
바쁜 하루 속에서도 문화를 곁눈질할 여유를 전하는 것이 목적입니다.

*참고: AI 뉴스 도메인은 과제가 지정한 6개 제외 매체와 겹쳐 선택지에서 빠졌다 (`f4c1c85` 이전 커밋 참고).*
...
| 소스 | 방식 | 전체 | 7일 내 | 본문 성공 | 평균 본문 | 최소 본문 | 발행 기여(33회) |
|---|---|---:|---:|---:|---:|---:|---:|
| 문화포털 전시 | API | 7 | 7 | 3/3 | 7,033자 | 2,142 | **37건** |
| Colossal | RSS | 10 | 10 | 3/3 | 2,327자 | 1,932 | 20건 |
| 오마이뉴스 문화 | RSS | 20 | 20 | 3/3 | 2,998자 | 2,636 | 17건 |
| 경향 문화 | RSS | 50 | 50 | 2/3 | 1,055자 | 423 | 14건 |
| 동아 문화 | RSS | 50 | 50 | 2/3 | 1,094자 | 268 | 12건 |
| 서울신문 라이프 | RSS | 28 | 28 | 3/3 | 2,267자 | 1,765 | 11건 |
| Guardian Books | RSS | 28 | 15 | 3/3 | 6,223자 | 5,139 | 6건 |
| Designboom | RSS | 10 | 10 | 3/3 | 3,903자 | 2,669 | 6건 |
| 월간디자인 | RSS | 10 | 10 | 3/3 | 3,187자 | 1,641 | 6건 |
...
| 월간 객석 | RSS | 8 | 4 | 3/3 | 4,430자 | 3,122 | 2건 |
| 독서신문 | RSS | 50 | 29 | 2/3 | 876자 | 286 | 1건 |
| WhatsOnNetflix | RSS | 50 | 50 | 3/3 | 6,467자 | 2,578 | 0건 |
| Decider | RSS | 10 | 10 | 3/3 | 3,597자 | 1,448 | 0건 |

### 탈락 — 46곳 (`settings.yaml` 하단 묘지에 사유와 함께 남겨 둠)

다시 넣지 않도록 **탈락 사유를 코드 옆에 적어 두었습니다.** 후보 110여 곳을 재서 17곳을 채택하고 나머지를 관문별로 묶었습니다. 아래는 이름과 걸린 관문만 추린 것이고, 수치와 판단 근거는 `settings.yaml` 묘지에 있습니다.
...
WhatsOnNetflix 는 7일 50건 · 본문 평균 6,467자로 지표가 가장 좋은 축인데 **33회 동안 발행 0건**입니다. 넷플릭스 신작 공지는 "이미 화제인 것" 과 "아직 볼 수 없는 것" 이라 중요도 기준 1·3번에서 떨어집니다. 관문은 *쓸 수 있는가*, 기여도는 *쓸 만한가* 를 잽니다. `report_card.py` 가 후자를 매 실행 쌓아 두고, 몇 주째 0이면 목록에서 빼라고 표시합니다.
...
| "한국어 '~합니다'체로" | 영어로 오거나 평서체로 온다 | `report()` 가 한글 포함 여부와 문장 끝을 검사해 `style_tries` 까지 재요청, 못 고치면 로그에 남긴다 |
...
#### 이 회차에서 드러난 흠 두 가지 — 그리고 고친 것

발행물을 눈으로 보면 설계대로 되지 않은 곳이 나옵니다. **둘 다 재 보고 고쳤습니다.**

**① 문체가 섞였습니다.** 4번 조선왕릉 요약이 *"문화축제가 **열린다**"*, *"'동구릉 야별행'**이다**"* 로 평서체입니다.

`report()` 가 문장 끝을 검사해 재요청했지만 2회 만에 못 고쳤습니다(`style_unfixed: 3`). 설계상 "못 고치면 로그에 남기고 발행" 이 정한 동작이고 그대로 돌았지만, **약속 자체가 약했습니다** — 8건 중 6건이 재요청까지 갔고 그중 절반이 실패했습니다.

상한을 올리기 전에 **정말 효과가 있는지 쟀습니다.** 상한만 올리면 토큰만 쓰고 안 고쳐질 수도 있으니까요. 실제로 실패했던 그 기사로 4회 반복:
...
고집스러운 한 건이 **5번째 호출에서** 풀렸습니다. `style_tries` 를 `settings.yaml` 로 빼고 5로 올렸습니다. 추가 호출은 필요할 때만 일어납니다.

바꾼 뒤 실측: **`style_unfixed` 3건 → 1건**, 소요 41.3초 → 53.6초. `권오철 화백 생애 첫 개인전` 이 `재요청 4회, 해결` 로 남았습니다 — 상한을 올린 이유가 그대로 기록에 보입니다.

**② 버릴 것이 하나 통과했습니다.** 1번 세네카 기사 요약에 *"종합베스트셀러 순위에서 '세네카…'가 1위를 차지했습니다"* 가 들어 있습니다. `버릴_것` 에 **"이미 화제인 작품의 또 다른 리뷰 — 넷플릭스 주간 1위, 베스트셀러 1위"** 가 명시돼 있는데 선별이 놓쳤습니다.

**막을 층이 없었습니다.**

| 층 | 왜 못 잡나 |
|---|---|
| 검수 `check()` | **원문에 진짜로 있는 사실**이라 할루시네이션이 아니다. "원문과 맞는가" 만 본다 |
| 후처리 `pick_for_publish()` | 쿼터와 매체 상한만 본다. 내용은 보지 않는다 |

> 검수는 *"원문과 맞는가"* 를 봅니다. *"실어도 되는가"* 는 선별만 봅니다.
> 두 질문은 다르고, 뒤쪽에는 **이중 안전장치가 없었습니다.**

그래서 `audience.yaml` 에 **`버릴_낱말`** 을 두고, 발행 직전에 `headline`·`summary` 를 결정론적으로 한 번 더 걸렀습니다. 수집 단계의 선필터가 하는 일을 발행 단계에서도 하는 셈입니다.

넓게 적으면 멀쩡한 기사가 죽으므로 **거의 언제나 버릴 것인 말만** 골랐고, 넣기 전에 실측했습니다.

| 대상 | 결과 |
|---|---|
| 새어나간 세네카 요약 | ✅ 검출 |
| 고정 후보의 시상식 기사 3건 | **3/3 검출** |
| 나머지 정상 후보 18건 | 오검출 **0** |
| 과거 발행 13건 | 잘못 걸리는 것 **0** |

```
− 니체-쇼펜하우어 이어 세네카 — 버릴 낱말('베스트셀러 순위')
```

**다만 이것으로 충분하지 않습니다.** 낱말로 안 잡히는 버릴 것(*"보도자료 표현만 있고 관찰이 없는 글"*)은 여전히 선별만 봅니다. 6절의 *발행 전 사람 승인* 이 필요한 이유가 여기 있습니다.
...
── 깔때기 (최근 7회) ──
  ...
  2026-09-15T16:02:43     97     8     8     5   예선·본선 8%
  2026-09-15T16:22:30     97     8     8     5   예선·본선 8%
  2026-09-15T16:28:07     97     8     7     4   예선·본선 8%
  2026-09-15T16:33:42     97     8     8     5   예선·본선 8%
  2026-09-15T16:34:24     91     8     8     4   예선·본선 9%
...
`report_card.py` 는 `.get()` 기본값으로 읽으므로, `quota_dropped` 같은 새 필드가 생겨도 **과거 7행과 새 스키마 26행을 함께 읽고 깨지지 않습니다.** (회귀 확인 완료 — 위 출력이 33회 전부를 읽은 결과입니다.)

### 5-4. 자동 검수와 예외 처리 — 무엇이 어디서 걸리나

| 층 | 무엇을 잡나 | 실패하면 |
|---|---|---|
| **G1 본문 관문** | 본문 450자 미만 | 그 기사만 버리고 로그에 남김. 재수집 없음 |
| **문체·언어 검사** (코드) | 영어 출력, 평서체 | `style_tries`(5)까지 재요청. 못 고치면 `style_unfixed` 로 기록하고 발행 |
| **검수** (LLM, 별도 호출) | 원문에 없는 사실·숫자·인용·과장 | 그 건만 탈락. 사유를 로그에 남김 |
| **후처리** | 배분 깨짐, 한 매체 독차지 | 상한 초과분을 사유와 함께 떨어뜨림 |
| **버릴 낱말** (코드) | 검수가 못 잡는 것 — 원문에 진짜 있는 '베스트셀러 1위' 따위 | 발행분에서 빼고 걸린 낱말을 로그에 남김 (5-2 ②) |
| **Discord 한도** | embed 총 6,000자 초과 | 보내기 전에 뒤에서부터 잘라냄 (400 방지) |
| **소스 장애** | 한 소스 타임아웃·404 | `try/except` 로 **그 소스만** 건너뛰고 사유를 로그에 실음 |
| **CI 접근 불가** | 공공 API 가 러너에서 안 닿음 | 국내에서 받아 둔 **캐시를 읽고** 나이를 로그에 남김. 캐시도 없으면 건너뜀 (5-2-2절) |
...
**⑦ 설정 문자열에 조사를 붙였다가 문장이 깨졌습니다.**
취재 프롬프트가 `f"당신은 {독자.누구}을 위한 뉴스레터 기자입니다."` 였습니다. `독자.누구` 가 *"…덜 알려진 것을 선호"* 로 끝나니 **"선호을 위한"** 이 됐습니다. 모델은 개의치 않았지만 사람이 프롬프트를 읽으면 바로 보입니다.

조사를 골라 붙이는 함수를 만들 수도 있었지만, **설정 문자열이 무엇으로 끝날지는 알 수 없습니다** — 한글이 아닐 수도, 괄호로 끝날 수도 있습니다. 그래서 문장 가운데 끼우지 않고 `[독자] …` 줄로 뗐습니다. 선별 프롬프트가 이미 그 형식이라 둘의 모양도 맞았습니다.

*설정과 코드를 잇는 자리에서는, 값이 문장의 일부가 되게 하지 말고 한 줄을 따로 주는 편이 안전합니다.*
...
| `store/metrics.jsonl` | 실행마다 한 줄 (33회) |
