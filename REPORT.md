...
처음 만든 것은 AI 뉴스 브리핑이었습니다(설정을 `audience.ai-news.yaml` · `settings.ai-news.yaml` 에 그대로 남겨 두었습니다). 두 가지 이유로 옮겼습니다.
...
`min_body` 를 600 → 500 으로 낮춘 것은 ArchDaily 때문입니다. 실측 평균 585자로 600 기준이면 아슬아슬하게 떨어집니다. 기준을 소스에 맞춰 낮춘 것이 아니라, **"본문이 있긴 한데 짧게 주는 매체" 와 "본문을 안 주는 매체" 를 가르는 선이 600이 아니라 500에 가깝다**는 실측에 따른 조정입니다. 탈락한 쪽은 452자(월간미술)와 0자(Dezeen·RogerEbert·NYT Books)로, 500 언저리에 몰려 있지 않습니다.

### 채택 — 12곳 (2026-09-15 12:20 실측, 창 168h, 본문 표본 3건)

| 소스 | 방식 | 전체 | 7일 내 | 본문 성공 | 평균 본문 | 최소 본문 | 발행 기여(20회) |
|---|---|---:|---:|---:|---:|---:|---:|
| 문화포털 전시 | API | 7 | 7 | 3/3 | 7,033자 | 2,142 | **12건** |
| Colossal | RSS | 10 | 10 | 3/3 | 2,327자 | 1,932 | **10건** |
| 경향 문화 | RSS | 50 | 50 | 2/3 | 1,055자 | 423 | 6건 |
| IndieWire | RSS | 12 | 12 | 3/3 | 6,168자 | 5,418 | 5건 |
| Designboom | RSS | 10 | 10 | 3/3 | 3,903자 | 2,669 | 5건 |
| Guardian Books | RSS | 28 | 15 | 3/3 | 6,223자 | 5,139 | 2건 |
| ArchDaily | RSS | 24 | 24 | 3/3 | 585자 | 518 | 2건 |
| Guardian Art | RSS | 23 | 17 | 2/3 | 4,619자 | 377 | 2건 |
| 월간디자인 | RSS | 10 | 10 | 3/3 | 3,187자 | 1,641 | 1건 |
| WhatsOnNetflix | RSS | 50 | 50 | 3/3 | 6,467자 | 2,578 | 0건 |
| Decider | RSS | 10 | 10 | 3/3 | 3,597자 | 1,448 | 0건 |
| LitHub | RSS | 10 | 10 | 3/3 | 3,438자 | 1,956 | 0건 |
...
| 월간미술 | 본문 452자 | G1 |
| 한겨레 문화면 | 30건인데 7일 내 0건, 날짜 표기 이상 | G2 |
| 브리크 매거진 | 7일 내 0건, 발행이 뜸함 | G2 |
| 씨네21 · 네오룩 · MMCA · 아트맵 · VMSPACE · KOPIS · 채널예스 · 넷플릭스 뉴스룸 | 404 또는 `entries` 0 | G3 |
...
WhatsOnNetflix 는 7일 50건 · 본문 평균 6,467자로 지표가 가장 좋은 축인데 **20회 동안 발행 0건**입니다. 넷플릭스 신작 공지는 "이미 화제인 것" 과 "아직 볼 수 없는 것" 이라 중요도 기준 1·3번에서 떨어집니다. 관문은 *쓸 수 있는가*, 기여도는 *쓸 만한가* 를 잽니다. `report_card.py` 가 후자를 매 실행 쌓아 두고, 몇 주째 0이면 목록에서 빼라고 표시합니다.
...
**④ 경향 문화는 선필터가 붙어 있습니다.**
국내 종합 문화면은 절반이 연예·가요입니다. LLM 을 부르기 전에 **비용 0의 결정론적 문자열 검사**로 좁힙니다(`filters.pick` 27개 키워드). 실측 로그: `· 선필터: 경향 문화 -18` — 매 실행 18건이 예선에 들어가기 전에 빠집니다.
...
후보는 매 실행 70건 안팎입니다. 한 번에 다 넣으면 두 가지가 나빠집니다.
...
실측 통과율: **70여 건 → 예선 16건 → 본선 8건 → 발행 5건 (약 11%)**

### 3-3. 왜 5건이 아니라 8건을 뽑는가 — 이번에 고친 것

**문제(실측)**: 쿼터는 `select()` 에 걸려 있는데, 그 뒤 `verify()` 가 건수를 **줄입니다**. 그래서 배분이 두 회차 연속 깨졌습니다.

| 회차 | 결과 | 무엇이 깨졌나 |
|---|---|---|
| 2026-09-14T16:42 | Designboom 2건 / 공간 2 · **문화 0** | 한 묶음이 통째로 비었다 |
| 2026-09-14T16:43 | 문화포털 전시 2건 / 문화 3 · **공간 0** | 반대쪽이 통째로 비었다 |
...
`run_id: 2026-09-15T12:03:45` · 소요 37.6초

```
   · 키: OPENAI_API_KEY=164자/sk-형 · DISCORD_WEBHOOK_URL=121자/URL형 · KCISA_SERVICE_KEY=64자/일반형
① 수집   전체 244 → 창(168h) 225 → 주제밖 -18 → 기발행 -6 → 중복 -0 → 상한 -130 → 후보 71건
   · 선필터: 경향 문화 -18
② 선별   71 → 8건 (여유분 8 · 최종 5건은 발행 직전 확정)
   쿼터(잠정): 문화 2/3 · 공간 6/2
   예선 묶음 40건 → 8건
   예선 묶음 31건 → 8건
   예선 통과 16건 (tier1 자동통과 0건 포함)
   · 기록으로 산책하기_서울의 공원 — 재요청 2회, 미해결(문체 불일치)
   − 대학에서 우리는 — 본문 부족(85자)
   · ‘DTF St. Louis’ Wins Emmy for Outsta — 재요청 2회, 미해결(문체 불일치)
③ 취재   7건 (본문 부족 -1)
④ 검수   합격 6 · 불합격 1
   − ‘DTF St. Louis’ Wins Emmy for Outstandin — 'DTF St. Louis'가 지난 월요일 밤 에미상에서
     수상했다는 내용은 맞지만, '눈에 띄는 수상 기록을 끝냈다'는 표현이 원문에 명시되지 않았다.;
     '이 시리즈는 2022년 이후 HBO에서 처음으로 이 상을 차…
⑤ 발행   합격 6 → 발행 5건 → dry-run
   쿼터: 문화 1/3 · 공간 4/2
   매체: 문화포털 전시 2 · Guardian Books 1 · Colossal 2
   − 사라 앤티스의 부드러운 파스텔, 신화, 꿈, 그리고 르네상스를 불 — 자리 없음(5건 초과)
...
| `① 수집` | 깔때기가 단계별로 몇 건을 걸렀는지. 상한(-130)이 중복(-0)보다 훨씬 많이 거른다 |
| `② 선별` | 여유분 8건. 쿼터는 "잠정" 이라고 명시 — 취재가 토픽을 다시 쓰므로 |
| `− 본문 부족(85자)` | G1 관문이 **실행 중에도** 동작한다. 모자란다고 다시 긁어 오지 않는다 |
| `재요청 2회, 미해결(문체 불일치)` | 부탁이 몇 번 중 몇 번 지켜지는가 — 이 실행은 2건 재요청, 2건 미해결 |
| `④ 검수` | **할루시네이션을 실제로 잡았다** — 원문에 없는 표현을 짚어 1건 탈락 |
| `⑤ 발행` | 후처리가 6 → 5건으로 줄이고 매체 2건 상한을 지켰다 |
...
── 소스별 발행 기여 (20회 실행 누적) ──
  ...
  LitHub            0건    ← 몇 주째 0이면 목록에서 뺀다

── 깔때기 (최근 7회) ──
  run_id                  수집    선별    취재    발행   통과율
  2026-09-14T16:43:53     72     5     4     3   예선·본선 7%
  2026-09-15T10:24:06     66     5     5     3   예선·본선 8%
...
> 위는 `report_card.py` 출력 그대로입니다. `OpenAI 블로그`·`전자신문 IT`·`AI타임스`·`ZDNet Korea`
> 네 줄은 도메인을 옮기기 전 AI 뉴스 시절의 누적 기록입니다 — 지금 소스 목록에는 없습니다.
> 깔때기 표에서 **선별이 5 → 8 로 바뀐 것이 여유분(`overselect`)을 적용한 지점**(11:58:24)입니다.

**여유분을 넣은 뒤 마지막 세 회차가 5 / 4 / 5건입니다.** 그 전 네 회차는 2 / 4 / 3 / 3건이었습니다. 목표 5건에 안정적으로 닿습니다.

`report_card.py` 는 `.get()` 기본값으로 읽으므로, `quota_dropped` 같은 새 필드가 생겨도 **과거 15행과 새 스키마 5행을 함께 읽고 깨지지 않습니다.** (회귀 확인 완료 — 위 출력이 20회 전부를 읽은 결과입니다.)
...
**② 쿼터를 앞에만 걸었다가 두 회차 연속 배분이 깨졌습니다.**
3-3절에 실측을 적었습니다. **보장은 결과가 정해지는 자리에서 걸어야 합니다.** 이 실수를 발견한 것은 로그가 `by_group` 을 매 실행 남기고 있었기 때문입니다 — 지표를 안 남겼으면 몇 주 뒤에나 알았을 것입니다.
...
| `store/metrics.jsonl` | 실행마다 한 줄 (20회) |
