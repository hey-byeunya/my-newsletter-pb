...
`run_id: 2026-09-15T15:35:34` · 소요 41.3초 · **HTTP 204** (성공, 본문 없음)

![디스코드 발행 화면](assets/discord.png)

머리에 `오늘의 곁눈질` 과 `2026년 09월 15일 · 볼 것 1 · 갈 곳 4` 가 찍혔습니다. 기사마다 왼쪽 띠 색이 다른 것은 토픽 색입니다 — 1번 `책`(보라), 2·3번 `전시·공간`(청록), 4번 `공연`(연두). 아래 꼬리표는 `출처 · 토픽` 입니다.
...
`store/published.jsonl` 에 발행 URL 5건이 기록됐습니다. 이 파일이 다음 실행에서 같은 기사를 다시 뽑지 않게 막습니다(위 로그의 `기발행 -6`).
...
`store/metrics.jsonl` 의 새 행: `verified 7 → published 5` · `quota_dropped 2` · `style_unfixed 3`.

#### 이 회차에서 보이는 흠 두 가지

발행물을 눈으로 보면 설계대로 되지 않은 곳이 드러납니다. 숨기지 않고 적습니다.

**① 문체가 섞였습니다.** 4번 조선왕릉 요약이 *"문화축제가 **열린다**"*, *"'동구릉 야별행'**이다**"* 로 평서체입니다. `report()` 가 문장 끝을 검사해 **2회 재요청했지만 못 고쳤고**(`style_unfixed: 3`), 설계대로 "못 고치면 로그에 남기고 발행" 했습니다. 로그에 `재요청 2회, 미해결(문체 불일치)` 로 정확히 남아 있습니다. **약속한 대로 동작했지만, 약속 자체가 약합니다** — 재요청 상한이 2회로 모자랍니다.

**② 버릴 것이 하나 통과했습니다.** 1번 세네카 기사 요약에 *"종합베스트셀러 순위에서 '세네카…'가 1위를 차지했습니다"* 가 들어 있습니다. `audience.yaml` 의 버릴 것에 **"이미 화제인 작품의 또 다른 리뷰 — 넷플릭스 주간 1위, 베스트셀러 1위"** 가 명시돼 있는데 선별이 놓쳤습니다. 검수는 이걸 잡지 못합니다 — **원문에 실제로 있는 사실**이라 할루시네이션이 아니기 때문입니다.

②가 드러내는 것은 층의 경계입니다. **검수는 "원문과 맞는가" 만 봅니다. "실어도 되는가" 는 선별이 봐야 하고, 선별이 놓치면 막을 곳이 없습니다.** 발행 직전 후처리도 쿼터와 매체 상한만 보지 내용은 보지 않습니다. 6절의 향후 개선에 적은 *발행 전 사람 승인* 이 필요한 이유가 여기 있습니다.
