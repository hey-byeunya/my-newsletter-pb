...
| 소스 | 방식 | 전체 | 7일 내 | 본문 성공 | 평균 본문 | 최소 본문 | 발행 기여(25회) |
|---|---|---:|---:|---:|---:|---:|---:|
| 문화포털 전시 | API | 7 | 7 | 3/3 | 7,033자 | 2,142 | **27건** |
| Colossal | RSS | 10 | 10 | 3/3 | 2,327자 | 1,932 | 18건 |
| 경향 문화 | RSS | 50 | 50 | 2/3 | 1,055자 | 423 | 14건 |
| 오마이뉴스 문화 | RSS | 20 | 20 | 3/3 | 2,998자 | 2,636 | 11건 |
...
| 동아 문화 | RSS | 50 | 50 | 2/3 | 1,094자 | 268 | 5건 |
| 월간디자인 | RSS | 10 | 10 | 3/3 | 3,187자 | 1,641 | 4건 |
| 서울신문 라이프 | RSS | 28 | 28 | 3/3 | 2,267자 | 1,765 | 4건 |
...
WhatsOnNetflix 는 7일 50건 · 본문 평균 6,467자로 지표가 가장 좋은 축인데 **25회 동안 발행 0건**입니다. 넷플릭스 신작 공지는 "이미 화제인 것" 과 "아직 볼 수 없는 것" 이라 중요도 기준 1·3번에서 떨어집니다. 관문은 *쓸 수 있는가*, 기여도는 *쓸 만한가* 를 잽니다. `report_card.py` 가 후자를 매 실행 쌓아 두고, 몇 주째 0이면 목록에서 빼라고 표시합니다.
...
`report_card.py` 는 `.get()` 기본값으로 읽으므로, `quota_dropped` 같은 새 필드가 생겨도 **과거 7행과 새 스키마 18행을 함께 읽고 깨지지 않습니다.** (회귀 확인 완료 — 위 출력이 25회 전부를 읽은 결과입니다.)
...
- **비교 실험은 있지만, 평가셋과 회귀 검사는 아직 없습니다.** 이 둘은 다른 것입니다. 지금 있는 것은 *방법* 입니다 — 고정 입력을 떠 두고, 정답을 손으로 붙이고, 변형별로 여러 번 돌려 평균을 본다. 실제로 세 번 썼습니다(3-6절 탈락 사유 · 3-7절 본선 요청 수와 같은 사건 묶기 · 5-4절 검수 프롬프트). 두 번은 바꿨고 **한 번은 "바꾸지 않는다" 가 결론**이었습니다.
  ...
  없는 것은 그 방법의 **자동화와 상설화** 입니다. 실험 스크립트는 저장소 밖 임시 폴더에 있고, 고정 입력도 그때그때 떠서 썼습니다. 그래서 *지금* 프롬프트를 고치면 **과거 실험을 다시 돌려 확인할 수 없습니다.** 다음 단계는 고정 입력과 정답표를 저장소에 넣고 한 명령으로 재현되게 만드는 것입니다.
...
| `store/metrics.jsonl` | 실행마다 한 줄 (25회) |
