...
|---|---|---:|---:|---:|---:|---:|---:|
| 문화포털 전시 | API | 7 | 7 | 3/3 | 7,033자 | 2,142 | **18건** |
...
| 월간디자인 | RSS | 10 | 10 | 3/3 | 3,187자 | 1,641 | 3건 |
...
`run_id: 2026-09-15T14:04:38` · 소요 39.4초 · 소스 17곳 · 쿼터 문화 2 · 공간 3

```
   · 키: OPENAI_API_KEY=164자/sk-형 · DISCORD_WEBHOOK_URL=121자/URL형 · KCISA_SERVICE_KEY=64자/일반형
① 수집   전체 400 → 창(168h) 356 → 주제밖 -80 → 기발행 -6 → 중복 -0 → 상한 -171 → 후보 99건
   · 선필터: 경향 문화 -18, 동아 문화 -31, 서울신문 라이프 -18, 오마이뉴스 문화 -13
② 선별   99 → 8건 (여유분 8 · 최종 5건은 발행 직전 확정)
   쿼터(잠정): 문화 3/2 · 공간 5/3
   예선 묶음 40건 → 8건
   예선 묶음 40건 → 8건
   예선 묶음 19건 → 3건
   예선 통과 19건 (tier1 자동통과 0건 포함)
   · 기록으로 산책하기_서울의 공원 — 재요청 1회, 해결
   · 권순익 ‘산에 비친 산…’ 展 — 재요청 1회, 해결
   − [신간] 『지상의 모든 심리 : 타인을 읽는 시간』 — 본문 부족(286자)
   · ‘한국판 쉰들러’ 문형순 서장 역에 최덕문… 영화 ‘부당하므로 불 — 재요청 1회, 해결
   · “폭력의 시대에 신념 지킨 이들 그리고 싶었다”…‘암살자(들)’  — 재요청 1회, 해결
③ 취재   7건 (본문 부족 -1)
④ 검수   합격 5 · 불합격 2
   − ACC 필름앤비디오 《아시아의 장치들》 — 요약에서 '관객에게 주도적이고 능동적인 여성들의 목소리를 전달하고자 합니다'라고 언급되었으나, 원문에는 이 문구와 관련된 명확한 표현이나 구체적인 주장이 없습니다.
   − 권순익 ‘산에 비친 산…’ 展 — 전시장 기간은 '다음 달 11일'이라고 명시되어 있으나, 요약에서는 '다음 달 11일까지'로 표현되어 있어 약간의 해석 차이가 있을 수 있지만 사실상 일치함.; 전시 제목에 대한 작가의 태도 설명이 요약에서 빠져 있
⑤ 발행   합격 5 → 발행 4건 → dry-run
   쿼터: 문화 2/2 · 공간 2/3
   매체: 문화포털 전시 1 · 서울신문 라이프 2 · 월간디자인 1
   − 뮤지컬로, 달밤 답사로 느끼는 세계유산 조선왕릉 — 매체 상한(서울신문 라이프 2건)
```
...
`report_card.py` 는 `.get()` 기본값으로 읽으므로, `quota_dropped` 같은 새 필드가 생겨도 **과거 7행과 새 스키마 11행을 함께 읽고 깨지지 않습니다.** (회귀 확인 완료 — 위 출력이 18회 전부를 읽은 결과입니다.)
