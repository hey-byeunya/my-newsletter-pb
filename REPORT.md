...
| 소스 | 방식 | 전체 | 7일 내 | 본문 성공 | 평균 본문 | 최소 본문 | 발행 기여(17회) |
|---|---|---:|---:|---:|---:|---:|---:|
| 문화포털 전시 | API | 7 | 7 | 3/3 | 7,033자 | 2,142 | **17건** |
| Colossal | RSS | 10 | 10 | 3/3 | 2,327자 | 1,932 | 12건 |
| 경향 문화 | RSS | 50 | 50 | 2/3 | 1,055자 | 423 | 8건 |
...
| 오마이뉴스 문화 | RSS | 20 | 20 | 3/3 | 2,998자 | 2,636 | 4건 |
...
| 서울신문 라이프 | RSS | 28 | 28 | 3/3 | 2,267자 | 1,765 | 1건 |
...
WhatsOnNetflix 는 7일 50건 · 본문 평균 6,467자로 지표가 가장 좋은 축인데 **17회 동안 발행 0건**입니다. 넷플릭스 신작 공지는 "이미 화제인 것" 과 "아직 볼 수 없는 것" 이라 중요도 기준 1·3번에서 떨어집니다. 관문은 *쓸 수 있는가*, 기여도는 *쓸 만한가* 를 잽니다. `report_card.py` 가 후자를 매 실행 쌓아 두고, 몇 주째 0이면 목록에서 빼라고 표시합니다.
...
`WhatsOnNetflix` 와 `Decider` 가 **둘 다 17회 누적 0건**인 것이 같은 이야기입니다. 이건 소스가 나쁘다기보다 **기준이 의도대로 작동하는 증거**입니다 — "지금 갈 수 있는 전시" 가 "다음 달 공개될 시리즈" 를 이기는 것은 이 독자에게 맞는 결과입니다. 다만 쿼터(문화 3)가 자주 안 채워지는 값을 치릅니다.
...
`run_id: 2026-09-15T13:08:44` · 소요 43.0초 · 소스 17곳

```
   · 키: OPENAI_API_KEY=164자/sk-형 · DISCORD_WEBHOOK_URL=121자/URL형 · KCISA_SERVICE_KEY=64자/일반형
① 수집   전체 400 → 창(168h) 356 → 주제밖 -79 → 기발행 -6 → 중복 -1 → 상한 -171 → 후보 99건
   · 선필터: 경향 문화 -18, 동아 문화 -31, 서울신문 라이프 -17, 오마이뉴스 문화 -13
② 선별   99 → 8건 (여유분 8 · 최종 5건은 발행 직전 확정)
   쿼터(잠정): 문화 3/3 · 공간 5/2
   예선 묶음 40건 → 8건
   예선 묶음 40건 → 8건
   예선 묶음 19건 → 8건
   예선 통과 24건 (tier1 자동통과 0건 포함)
   − ‘He would have been the center of everyt — 자리 없음(묶음 공간)
   − Sara Anstis’ Soft Pastels Summon Mytholo — 자리 없음(묶음 공간)
   − Houston Center for Contemporary Craft Ce — 자리 없음(묶음 공간)
   − 전 세계 커뮤니티센터 네 곳이 ‘포용’을 디자인하는 방식 — 자리 없음(묶음 공간)
   − ‘When Hope Calls’ Star Cindy Busby Talks — 자리 없음(묶음 문화)
   − We Are Family: photography from Images V — 자리 없음(묶음 문화)
   − 재미로 하는 독서의 힘…만화·잡지도 뇌 건강에 도움 [노화설계] — 자리 없음(묶음 문화)
   − 교보문고 ‘이지페이지’ 3차 도서 출간 — 자리 없음(묶음 문화)
   · 권순익 ‘산에 비친 산…’ 展 — 재요청 1회, 해결
   · 육영수 여사 저격범은 혼자였나···52년 전 진실 좇는 ‘암살자( — 재요청 1회, 해결
   · '반세기의 열정, 화폭에 담다'... 권오철 화백 생애 첫 개인전 — 재요청 1회, 해결
   · 프레드리히 쿤나스의 '가장 가까운 먼 곳'에 예술 역사와 대중 문 — 재요청 1회, 해결
③ 취재   8건
④ 검수   합격 6 · 불합격 2
   − 권순익 ‘산에 비친 산…’ 展 — 전시 제목 '산에 비친 산, 물에 비친 물'은 원문에서 언급되었으나, 그 의미 설명이 원문에 구체적으로 담겨 있지 않다.; 전시 개막일이 '지난달 29일'로 되어 있어서 '10월 29일'로 잘못 특정되었다.; 전시 
   − ACC 필름앤비디오 《아시아의 장치들》 — 요약에서 'ACC가 10주년을 맞아 아시아 각 지역의 역사와 정체성을 재조명하고, 공동체적 서사를 전합니다.'라는 주장은 원문에서 'ACC 10주년을 맞아 기획된 전시'와 '서로 다른 정체성과 역사를 지닌 복수의 아
⑤ 발행   합격 6 → 발행 5건 → dry-run
   쿼터: 문화 2/3 · 공간 3/2
   매체: 경향 문화 1 · Guardian Books 1 · LitHub 1 · 오마이뉴스 문화 1 · Colossal 1
   − 분 Branching Structure, Pakistan에 있는  — 자리 없음(5건 초과)
```
...
| `① 수집` | 깔때기가 단계별로 몇 건을 걸렀는지. **상한(-171)이 중복(-1)보다 훨씬 많이 거른다** — 7일 창이라 한 소스가 쉽게 독차지한다 |
| `· 선필터:` | 국내 종합지 4곳에서 **79건**이 LLM 을 부르기 전에 빠진다 — 비용 0의 결정론적 검사 |
| `예선 묶음 40 / 40 / 19` | 소스를 17곳으로 늘려 묶음이 3개가 됐다. 늘어난 호출이 국내 기사를 데려오는 값이다 |
| `② 선별 … 쿼터(잠정): 문화 3/3` | **국내 문화 소스를 더한 효과.** 직전 회차들은 문화가 0~1건이었다 |
| `− … 자리 없음(묶음 문화/공간)` | 탈락 사유에 **어느 묶음에서 밀렸는지**가 남는다. 양쪽에서 고르게 밀렸다 |
| `· … 재요청 1회, 해결` | 문체를 코드로 확인하고 다시 요청한다. 이 실행은 4건 재요청, **전부 해결** |
| `④ 검수   합격 6 · 불합격 2` | **할루시네이션을 2건 잡았다.** 둘 다 국내 전시 기사다 — `'지난달 29일' 을 '10월 29일' 로 잘못 특정` |
| `⑤ 발행   합격 6 → 발행 5건` | 후처리가 6건을 5건으로 줄였다(`quota_dropped: 1`). 탈락분도 사유와 함께 남는다 |
| `매체: … 1 · 1 · 1 · 1 · 1` | 매체 5곳에 1건씩 — 상한 2를 넘지 않았고, 그중 **2건이 국내 소스**다 |
...
`report_card.py` 는 `.get()` 기본값으로 읽으므로, `quota_dropped` 같은 새 필드가 생겨도 **과거 7행과 새 스키마 10행을 함께 읽고 깨지지 않습니다.** (회귀 확인 완료 — 위 출력이 17회 전부를 읽은 결과입니다.)
...
| `store/metrics.jsonl` | 실행마다 한 줄 (17회) |
