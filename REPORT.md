...
| 소스 | 방식 | 전체 | 7일 내 | 본문 성공 | 평균 본문 | 최소 본문 | 발행 기여(30회) |
|---|---|---:|---:|---:|---:|---:|---:|
| 문화포털 전시 | API | 7 | 7 | 3/3 | 7,033자 | 2,142 | **32건** |
| Colossal | RSS | 10 | 10 | 3/3 | 2,327자 | 1,932 | 20건 |
| 오마이뉴스 문화 | RSS | 20 | 20 | 3/3 | 2,998자 | 2,636 | 17건 |
| 경향 문화 | RSS | 50 | 50 | 2/3 | 1,055자 | 423 | 14건 |
| 동아 문화 | RSS | 50 | 50 | 2/3 | 1,094자 | 268 | 10건 |
| 서울신문 라이프 | RSS | 28 | 28 | 3/3 | 2,267자 | 1,765 | 7건 |
...
WhatsOnNetflix 는 7일 50건 · 본문 평균 6,467자로 지표가 가장 좋은 축인데 **30회 동안 발행 0건**입니다. 넷플릭스 신작 공지는 "이미 화제인 것" 과 "아직 볼 수 없는 것" 이라 중요도 기준 1·3번에서 떨어집니다. 관문은 *쓸 수 있는가*, 기여도는 *쓸 만한가* 를 잽니다. `report_card.py` 가 후자를 매 실행 쌓아 두고, 몇 주째 0이면 목록에서 빼라고 표시합니다.
...
`report_card.py` 는 `.get()` 기본값으로 읽으므로, `quota_dropped` 같은 새 필드가 생겨도 **과거 7행과 새 스키마 23행을 함께 읽고 깨지지 않습니다.** (회귀 확인 완료 — 위 출력이 30회 전부를 읽은 결과입니다.)
...
| `store/metrics.jsonl` | 실행마다 한 줄 (30회) |
