...
| | |
|---|---|
| 분야 | 문화 콘텐츠(OTT·영화·책) + 공간(전시·건축·디자인) |
| 소스 | **16곳 — 국내 7 · 해외 9** (RSS 15 + 공공 API 1) |
| 파이프라인 | 수집 → 선별 → 취재 → 검수 → 발행 (LangGraph 노드 5개) |
| 발행처 | Discord Webhook (embed 6개 = 리드 1 + 기사 5) |
| 누적 실행 | 14회 (`store/metrics.jsonl`) |
...
묶음설명:
  ...
  공간: "직접 찾아가서 몸으로 겪는 것. 전시장·미술관·건축물"
```
...
### 채택 — 16곳 (2026-09-15 12:41 실측, 창 168h, 본문 표본 3건)

| 소스 | 방식 | 전체 | 7일 내 | 본문 성공 | 평균 본문 | 최소 본문 | 발행 기여(14회) |
|---|---|---:|---:|---:|---:|---:|---:|
| 문화포털 전시 | API | 7 | 7 | 3/3 | 7,033자 | 2,142 | **15건** |
| Colossal | RSS | 10 | 10 | 3/3 | 2,327자 | 1,932 | 10건 |
| 경향 문화 | RSS | 50 | 50 | 2/3 | 1,055자 | 423 | 7건 |
| IndieWire | RSS | 12 | 12 | 3/3 | 5,515자 | 4,626 | 5건 |
| Designboom | RSS | 10 | 10 | 3/3 | 3,903자 | 2,669 | 5건 |
| Guardian Books | RSS | 28 | 15 | 3/3 | 6,223자 | 5,139 | 4건 |
| ArchDaily | RSS | 24 | 24 | 3/3 | 585자 | 518 | 2건 |
| Guardian Art | RSS | 23 | 17 | 2/3 | 4,619자 | 377 | 2건 |
| 월간디자인 | RSS | 10 | 10 | 3/3 | 3,187자 | 1,641 | 2건 |
| LitHub | RSS | 10 | 10 | 3/3 | 3,438자 | 1,956 | 1건 |
| 동아 문화 | RSS | 50 | 50 | 2/3 | 1,094자 | 268 | 1건 |
| 오마이뉴스 문화 | RSS | 20 | 20 | 3/3 | 2,998자 | 2,636 | 1건 |
| WhatsOnNetflix | RSS | 50 | 50 | 3/3 | 6,467자 | 2,578 | 0건 |
| Decider | RSS | 10 | 10 | 3/3 | 3,597자 | 1,448 | 0건 |
| 월간 객석 | RSS | 8 | 8 | 3/3 | 4,430자 | 3,122 | 0건 |
| 서울신문 라이프 | RSS | 26 | 26 | 3/3 | 2,267자 | 1,765 | 0건 |
...
국내 비중을 늘리려고 후보 **70여 곳**을 재면서, G1~G3 와는 **다른 축**에서 떨어지는 소스들이 나왔습니다. 접근되고, 기사가 있고, 본문도 길게 주는데 — **싣는 내용이 이 뉴스레터의 것이 아닌** 경우입니다.
...
WhatsOnNetflix 는 7일 50건 · 본문 평균 6,467자로 지표가 가장 좋은 축인데 **14회 동안 발행 0건**입니다. 넷플릭스 신작 공지는 "이미 화제인 것" 과 "아직 볼 수 없는 것" 이라 중요도 기준 1·3번에서 떨어집니다. 관문은 *쓸 수 있는가*, 기여도는 *쓸 만한가* 를 잽니다. `report_card.py` 가 후자를 매 실행 쌓아 두고, 몇 주째 0이면 목록에서 빼라고 표시합니다.

**③ 사다리를 한 칸 올라간 소스가 1등입니다.**
국내 전시는 **RSS 가 없습니다**. 매체 RSS(2칸)로는 못 구해서 공공 API(1칸, 문화포털 `cultureinfo`)까지 올라갔고, 그 소스가 발행 기여 1위(10건)입니다. 대신 대가가 있습니다 — GitHub Actions 러너(해외 리전)에서는 이 주소에 닿지 않습니다(로컬 0.9초 성공 / 러너 ConnectTimeout 30초 × 3회). 매일 90초를 헛쓰지 않도록 `local_only: true` 로 표시해 **CI 에서는 건너뛰고 건너뛴 사실만 로그에 남깁니다.**
...
**⑤ 국내 쪽에 무게를 실었습니다.**
독자가 실제로 갈 수 있는 곳은 국내입니다. 그런데 초기 목록은 해외 9 · 국내 3 이었습니다 — 영어권 매체가 RSS 를 잘 내기 때문이지, 독자에게 더 쓸모 있어서가 아니었습니다. 후보 **70여 곳**을 재서 국내 4곳을 더했습니다(**국내 7 · 해외 9**).
...
후보는 매 실행 **95건 안팎**입니다(국내 소스를 더하기 전에는 71건이었습니다). 한 번에 다 넣으면 두 가지가 나빠집니다.
...
실측 통과율: **95건 → 예선 20건 → 본선 8건 → 발행 5건 (약 8%)**

소스를 16곳으로 늘리자 예선 묶음이 2개에서 **3개**로 늘었습니다(`40 + 40 + 15`). 예선 LLM 호출이 한 번 더 늘어난 값을 치르고, 그 대신 국내 기사가 후보에 들어옵니다.

### 3-3. 왜 5건이 아니라 8건을 뽑는가 — 이번에 고친 것

**문제(실측)**: 쿼터는 `select()` 에 걸려 있는데, 그 뒤 `verify()` 가 건수를 **줄입니다**. 앞에서 문화 3 · 공간 2 를 맞춰 놓아도, 검수가 그중 몇 건을 떨어뜨리면 발행 시점에는 배분이 이미 깨져 있습니다.

`store/metrics.jsonl` 의 적용 전 7회 전부입니다 (쿼터 문화 3 · 공간 2, 목표 5건):
...
| "토픽은 이 다섯 중 하나" | '전시·공간' 대신 '전시' 가 와서 색이 기본값으로 떨어졌다 | 구조화 출력 스키마에 `enum` 으로 못박는다 |
...
아래 두 블록은 **`run_id: 2026-09-15T12:03:45`** — 국내 소스를 더하기 전(소스 12곳·후보 71건)의 회차입니다. 이 회차를 고른 이유는 **매체 상한이 실제로 무언가를 떨어뜨린 유일한 기록**이기 때문입니다. 현재 설정(소스 16곳)의 로그는 5-1절에 전체를 실었습니다.
...
`run_id: 2026-09-15T12:42:14` · 소요 43.3초 · 소스 16곳

```
   · 키: OPENAI_API_KEY=164자/sk-형 · DISCORD_WEBHOOK_URL=121자/URL형 · KCISA_SERVICE_KEY=64자/일반형
① 수집   전체 348 → 창(168h) 329 → 주제밖 -79 → 기발행 -6 → 중복 -0 → 상한 -149 → 후보 95건
   · 선필터: 경향 문화 -18, 동아 문화 -31, 서울신문 라이프 -17, 오마이뉴스 문화 -13
② 선별   95 → 8건 (여유분 8 · 최종 5건은 발행 직전 확정)
   쿼터(잠정): 문화 1/3 · 공간 7/2
   예선 묶음 40건 → 8건
   예선 묶음 40건 → 8건
   예선 묶음 15건 → 4건
   예선 통과 20건 (tier1 자동통과 0건 포함)
   − Festival of the Winds 2026: splashes of  — 자리 없음(묶음 공간)
   − 2025 공공수장고 야외 전시 프로젝트 《강주현: 연결의 비정형》 — 자리 없음(묶음 공간)
   − 2026 이응노미술관 기획전 《이응노·김창열》 — 자리 없음(묶음 공간)
   − Anna Ortiz Traverses the Multiple Realit — 자리 없음(묶음 공간)
   − Sara Anstis’ Soft Pastels Summon Mytholo — 자리 없음(묶음 공간)
   − (문화포털 전시) 2026 이응노미술관 기획전 《이응노·김창열》
   − (오마이뉴스 문화) 인천 괭이부리 마을을 표현한 10명의 작가들
   − (Designboom) luminous globes extend from branched metal tree installation in pakistan
   − (Colossal) Art History and Pop Culture Converge in Frederich Kunath’s ‘The Nearest Faraway Place’
   − (Colossal) Houston Center for Contemporary Craft Celebrates 25th Anniversary With Silver Exhibition
   − (서울신문 라이프) ‘조선왕릉축전’ 야간답사 갈까…최재림 뮤지컬까지
   − (Guardian Art) Festival of the Winds 2026: splashes of colour in the sky and on the sand – in pictures
   − (Decider) Emmys 2026 Red Carpet: Where to Watch For Free
   − (ArchDaily) Canopy House / Lamoureux Architect Inc.
   − (서울신문 라이프) 안은진 “양치하는데 물이 새” 이상해진 얼굴…촬영 멈추게 한 ‘이 병’ [라이프]
   · 기록으로 산책하기_서울의 공원 — 재요청 1회, 해결
   − 대학에서 우리는 — 본문 부족(85자)
   · Emily St John Mandel: ‘When half the — 재요청 2회, 해결
   · 인천의 인문풍경 네 번째 이야기 <기억의 땅, 공간의 흔적> 전( — 재요청 1회, 해결
   · 권순익 ‘산에 비친 산…’ 展 — 재요청 1회, 해결
   · 로에베와 박종진 작가가 쌓아 올린 ‘시간의 층위’ — 재요청 1회, 해결
   · 뮤지컬로, 달밤 답사로 느끼는 세계유산 조선왕릉 — 재요청 2회, 미해결(문체 불일치)
③ 취재   7건 (본문 부족 -1)
④ 검수   합격 5 · 불합격 2
   − ‘He would have been the center of everyt — 전시의 제목이 'If Emmett Till Lived: Freedom on American Ground'로 정확하게 명시되어 있지 않음.; 전시의 공동 큐레이터로 하버드 교수 사라 엘리자베스 루이스가 포함되었지만 '
   − 뮤지컬로, 달밤 답사로 느끼는 세계유산 조선왕릉 — 개최지가 12곳으로 늘어난 것이지만, '야간 답사'와 '능침 특별 개방' 등의 프로그램이 포함되었는지는 원문에서 확인할 수 없음. 또한, 세줄 요약에서 '조선왕릉축전, 다음달 16일 융릉·건릉 개막'이라고 명시했는데
⑤ 발행   합격 5 → 발행 5건 → dry-run
   쿼터: 문화 1/3 · 공간 4/2
   매체: 문화포털 전시 1 · Guardian Books 1 · 오마이뉴스 문화 1 · 동아 문화 1 · 월간디자인 1
```
...
| `① 수집` | 깔때기가 단계별로 몇 건을 걸렀는지. 상한(-149)이 중복(-0)보다 훨씬 많이 거른다 |
| `· 선필터:` | 국내 종합지 4곳에서 **79건**이 LLM 을 부르기 전에 빠진다 — 비용 0의 결정론적 검사 |
| `예선 묶음 40 / 40 / 15` | 소스를 16곳으로 늘려 묶음이 3개가 됐다. 늘어난 호출이 국내 기사를 데려오는 값이다 |
| `② 선별` | 여유분 8건. 쿼터는 "잠정" 이라고 명시 — 취재가 토픽을 다시 쓰므로 |
| `− … 자리 없음(묶음 공간)` | 공간 후보가 7건이나 몰린 날이다. **탈락 사유에 어느 묶음에서 밀렸는지가 남는다** |
| `− 대학에서 우리는 — 본문 부족(85자)` | G1 관문이 **실행 중에도** 동작한다. 모자란다고 다시 긁어 오지 않는다 |
| `재요청 2회, 미해결(문체 불일치)` | 부탁이 몇 번 중 몇 번 지켜지는가 — 이 실행은 6건 재요청, 1건 미해결 |
| `④ 검수` | **할루시네이션을 2건 잡았다.** 국내 기사에서도 잡힌다 — "야간 답사·능침 특별 개방이 원문에서 확인되지 않음" |
| `⑤ 발행` | 매체 5곳에 1건씩 — 상한 2를 넘지 않았고, **5건 중 4건이 국내 소스**다 |
...
── 소스별 발행 기여 (14회 실행 누적) ──
  ...
  서울신문 라이프          0건    ← 몇 주째 0이면 목록에서 뺀다

── 깔때기 (최근 7회) ──
  run_id                  수집    선별    취재    발행   통과율
  2026-09-15T11:58:24     71     8     7     5   예선·본선 11%
  2026-09-15T12:02:18     71     8     7     4   예선·본선 11%
  2026-09-15T12:03:45     71     8     7     5   예선·본선 11%
  2026-09-15T12:22:08     71     8     7     4   예선·본선 11%
  2026-09-15T12:23:19     71     8     8     5   예선·본선 11%
  2026-09-15T12:26:44     71     8     7     5   예선·본선 11%
  2026-09-15T12:42:14     95     8     7     5   예선·본선 8%
...
`report_card.py` 는 `.get()` 기본값으로 읽으므로, `quota_dropped` 같은 새 필드가 생겨도 **과거 7행과 새 스키마 7행을 함께 읽고 깨지지 않습니다.** (회귀 확인 완료 — 위 출력이 14회 전부를 읽은 결과입니다.)
...
이 실수를 발견한 것은 로그가 `by_group` 과 `by_source` 를 매 실행 남기고 있었기 때문입니다 — 지표를 안 남겼으면 몇 주 뒤에나 알았을 것입니다. 보고서를 쓰면서도 같은 일이 한 번 더 있었습니다. 처음에는 진단 문서에 적힌 대로 **회차 번호를 옮겨 적었다가**, `metrics.jsonl` 과 대조해 보니 제가 지목한 두 회차가 아니었습니다. 기록이 남아 있었기 때문에 표를 실제 7회 전부로 다시 쓸 수 있었습니다. *기억이 아니라 기록을 인용해야 합니다.*
...
| `store/metrics.jsonl` | 실행마다 한 줄 (14회) |
| `store/source_audit.json` | 소스 실측 원자료 |
| `docs/발행_단계_개선점과_해결방안.md` | 3-3절 개선의 진단 문서 |
